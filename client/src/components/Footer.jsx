  ...
  return (
    <footer className="bg-black text-white py-24 px-6 border-t border-white/5 relative overflow-hidden">
      ...
      <div className="max-w-7xl mx-auto flex flex-col items-center text-center relative z-10">
        ...
        {/* Navigation Grid */}
        <nav className="flex flex-wrap justify-center gap-x-16 gap-y-6 mb-20">
          {['Home', 'Members', 'Projects', 'Task', 'Rankings'].map((item) => (
            <a 
              key={item} 
              href={item === 'Home' ? '/' : `/${item.toLowerCase().replace(' ', '')}`} 
              className="text-[11px] font-black uppercase tracking-[0.4em] text-zinc-400 hover:text-white transition-colors relative group"
            >
              {item}
